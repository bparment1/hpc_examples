...
#DATE MODIFIED: 01/23/2017
#Version: 1
#PROJECT: PEATBOGS SESYNC
#TO DO:
#
#COMMIT: slurm options with rslurm 
#
#################################################################################################
...
sjob1 <- slurm_call(f=ftest, params=pars)
...
## To run job over multiple nodes (array job) use slurm_apply

sjob2 <- slurm_apply(ftest, pars,jobname="slurm_test1")
print_job_status(sjob2)

## send your to multiple nodes:
#slurm_apply(f=ftest, 
#            params=pars, 
#            jobname = "parallel_test1", 
#            nodes = 3, 
#            cpus_per_node = num_cores, #set to two
#            add_objects = NULL, 
#            pkgs = rev(.packages()), 
#            libPaths = NULL,
#            slurm_options = list(), 
...
##Note the keyword option "--array=0-2" that is used to launch three jobs  on a node.
